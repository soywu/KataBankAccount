...
/**
 * Bank Main System. Using Singleton pattern
 */
public class Main {
	...
	public BankAccount getAccount(int accountId) throws BankAccountNotFoundException {
		...
		if (ac == null)
			throw new BankAccountNotFoundException("Account not found for Id: " + accountId);
		return ac;
	}

	public BankRecord takeDeposit(int accountId, BigDecimal amount)
			throws BankAccountNotFoundException, InvalideTransactionException {
		BankAccount ac = getAccount(accountId);
		Deposit de = Deposit.of(amount, accountId);
		return ac.acceptTransaction(de);
	}

	public BankRecord makeWithdraw(int accountId, BigDecimal amount)
			throws BankAccountNotFoundException, InvalideTransactionException {
		BankAccount ac = getAccount(accountId);
